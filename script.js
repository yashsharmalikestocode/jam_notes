/* ===========================
   ...
   Matches your two-panel layout (Add / Browse)
   Auth fixed: immediate UI switch on login
   =========================== */

// ---------- Supabase init (with explicit auth options) ----------
...
/* ===========================
   AUTH (fixed)
   - Immediate UI update after sign in
   - Clear error handling
   - Busy state while calling API
=========================== */

function setAuthBusy(isBusy){
  $("#btn-login")?.toggleAttribute("disabled", isBusy);
  $("#btn-signup")?.toggleAttribute("disabled", isBusy);
}
...
$("#btn-signup").addEventListener("click", async ()=>{
  ...
  alert("Sign up successful. If confirmations are on, verify your email, then log in.");
});
...
$("#btn-login").addEventListener("click", async ()=>{
  ...
  if(error){
    // Surface common causes helpfully
    if (error.message?.toLowerCase().includes("email not confirmed")){
      alert("Your email is not confirmed yet. Please confirm from your inbox, then try again.");
    } else {
      alert(error.message);
    }
    return;
  ...
  // 🔑 Immediately switch UI without waiting for the onAuthStateChange callback
  currentUser = data.user ?? data.session?.user ?? null;
  if(currentUser){
    $("#auth-fields").classList.add("hidden");
    $("#auth-session").classList.remove("hidden");
    $("#session-email").textContent = `Signed in as ${currentUser.email}`;
    await afterLoginBoot();
  }else{
    // Fallback: refresh (covers rare edge cases)
...
// Keep in sync with token refreshes / tab changes
supabase.auth.onAuthStateChange(() => {
  // Do not rely solely on this (we also flip UI immediately on login),
  // but keep it to track refreshes/other tabs.
  refreshSessionUI();
});

/* ===========================
   POST-LOGIN BOOT & REALTIME
=========================== */
...
function renderTopicList(notes){
  ...
  const html = Array.from(byTopic.entries())
  ...
  for(const n of incoming){
    ...
    if(!ex || new Date(n.updated_at) > new Date(ex.updated_at)){
      upserts.push({
        ...
        title:n.title||"", question_id:n.question_id||"",
        topics:n.topics||[], refs:n.refs||"", body:n.body||""
      });
    }
  }
