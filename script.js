...
function setAuthState(isLoggedIn){
  const box = $("#auth");
  box.classList.toggle("logged-in",  isLoggedIn);
  box.classList.toggle("logged-out", !isLoggedIn);
}
...
$("#btn-signup").addEventListener("click", async ()=>{
  ...
  alert("Sign up successful. Confirm your email if required, then log in.");
});
...
$("#q").addEventListener("input", ()=>debounce(()=>runSearch("#q", "#results"), 140));
$("#q-browse").addEventListener("input", ()=>debounce(()=>runSearch("#q-browse", "#results-browse"), 140));
...
function renderResults(targetSel, list){
  ...
  if(!list.length){
    container.innerHTML = `<div class="result empty">No matches. Try <b>topic:Algebra</b> or <b>qid:2023</b>.</div>`;
    return;
  }
...
function buildTopicChips(notes){
  ...
  box.innerHTML = arr.length
    ? arr.map(t=>`<span class="chip ${t===activeTopicFilter?'active':''}" data-chip="${escapeAttr(t)}">${escapeHtml(t)}</span>`).join("")
...
function renderTopicList(notes){
  ...
  if(!notes.length){ wrap.innerHTML = `<div class="result empty">No notes yet.</div>`; return; }
...
$("#exportBtn").addEventListener("click", ()=>{
  ...
  const blob = new Blob(
    [JSON.stringify({ exported_at:new Date().toISOString(), notes: allNotes }, null, 2)],
    { type: "application/json" }
  );
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url; a.download = "jam-notes-export.json";
  a.click();
...
$("#importFile").addEventListener("change", async (e)=>{
  ...
  for(const n of incoming){
    ...
    if(!ex || new Date(n.updated_at) > new Date(ex.updated_at)){
      upserts.push({
        id:n.id, user_id: currentUser.id, title:n.title||"",
        question_id:n.question_id||"", topics:n.topics||[], refs:n.refs||"", body:n.body||""
      });
    }
  }
